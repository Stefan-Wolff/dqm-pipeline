...
class CorrectOrgs:
	...
	def run(self, dataFrames, spark):
		...
		for id_type, config in sources.items():
			...
			# join & take name
			id_field = "r_" + config["id_field"]
			if "." in id_field:
				id_field = "`" + id_field + "`"
				
			name_field = "r_" + config["name_field"]
			
			df_joined = df_joined.join(df_right, (df_joined["type"] == lit(id_type)) & \
											     (df_joined["id"] == df_right[id_field])
									, 'left')	\
								.withColumn("name", when(col(name_field).isNotNull(), col(name_field))	\
													.otherwise(col("name")))	\
